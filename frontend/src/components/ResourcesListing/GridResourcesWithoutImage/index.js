...
import styles from './gridResourcesWithoutImage.module.css';
import classNames from 'classnames';
import { groupBy } from '@/utils/groupBy';
import Link from '@/components/Link';
import { getResourcesSlug } from '@/utils/getResourcesSlug';
const GridResourcesWithoutImage = ({ resourcesHeading, resources, resourcesListingBackground, resourcesCardBackgroundColor, resourcesCardHoverColor = "#FFFFFF", isFilterTabs, isSearchBox, resourcesVerticalSpacing = "vertical_spacing_3", isPagination, resourcesListingLayout, resourcesListingSortBy = "default_adding" }) => {
    const [resourcesData, setResourcesData] = useState({});
    const [selectedTab, setSelectedTab] = useState('All');
    const [searchText, setSearchText] = useState('');
    const [searchedResult, setSearchedResult] = useState([]);
    const [currentPage, setCurrentPage] = useState(1);
    const [pageSize, setPageSize] = useState(isPagination ? 12 : 1000);
    ...
    return (
        <div className={classNames("container_padding")} style={{ background: resourcesListingBackground }}>
            <section className={classNames('section-spacing', `${resourcesVerticalSpacing}`.toLowerCase())} >
                ...
                <div className={classNames("row-extra-spacing", styles.card_gutter)}>
                    {
                        ...
                        filteredResources?.slice((currentPage - 1) * pageSize, currentPage * pageSize)?.map((resourceItem, index) => (
                            <div key={index}
                                className={classNames(styles.resource_card)}
                                onMouseEnter={() => setCurrent(index)}
                                onMouseLeave={() => setCurrent(null)}
                                style={current === index ? { background: resourcesCardHoverColor } : { background: resourcesCardBackgroundColor }}>
...
export default GridResourcesWithoutImage;